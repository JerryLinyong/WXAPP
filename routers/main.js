...
import { StyleSheet,  View, Image ,Text , StatusBar,DeviceEventEmitter } from 'react-native';
...
import MyPage from "../modules/my"
import { mapThemeStateToProps,mapThemeDispatchToProps } from '../store/common/theme/utils'
        ...
        if(focused){
            return (
                <View>
                    ...
                    {redDot ? <Text style={styles.redDot}/> : null}
                </View>
                )
        }else{
            return (
        ...
        render(){            
            ...
            return (
                <View>
                    ...
                    />
                    <ThemeingPage showRedDot={this.showRedDot} ref={this.pageRef} navigation={options.navigation} />
                </View>
            )
        }
    })
...
function createMainNavigator(props){
    let { theme, changeTheme ,redDot } = props
    const MainNavigator = createMaterialTopTabNavigator(
        {
            Home: { screen: TabbedPage(HomePage,props) },
            Message: { screen: TabbedPage(MessagePage,props) },
            Map: { screen: TabbedPage(MapPage,props) },
            Report: { screen: TabbedPage(ReportPage,props) },
            My: { screen: TabbedPage(MyPage,props) }
        },
        {
...
class MainPage extends React.Component {
    ...
    render(){
        const MainNavigator=createMainNavigator(Object.assign({},this.props,{redDotVisible:this.state.redDotVisible}))
        return(
            <MainNavigator/>
        )            
    }
...
export default connect(mapThemeStateToProps,mapThemeDispatchToProps)(MainPage)